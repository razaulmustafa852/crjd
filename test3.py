...
def topology():
    ...
    #info("*** Running CLI\n")
    time.sleep(05)
    subfolder = '_host_' + str(h) + '_band_' + str(b) +  '_delay_' + str(d) + '_ploss_' + str(p) 
    os.system('mkdir -p ' + subfolder)
    for i in range(h):
        stt='sta%s' % (i+1)
        stta=net.get(stt)
        print stta.cmd('cd /home/raza/goDash/DashApp/src/goDASH/_host_' + str(h) + '_band_' + str(b) +  '_delay_' + str(d) + '_ploss_' + str(p) + ' && sudo tcpdump -i '+str(stta)+'-eth0  -w '+str(stta)+'.pcap &')
...
def cap2(h,b,d,p):
    print os.system('sudo tcpdump -i s3-eth100 -U -w /home/raza/goDash/DashApp/src/goDASH/pcap/_host_'+str(h)+'_band_'+str(b) +'_delay_'+str(d)+'_ploss'+str(p)+'_ap.pcap ') 



def test1(num,h,b,d,p):
   
   
    print num.cmd('cd /home/raza/goDash/DashApp/src/goDASH/ && ./goDASH --config ../config/configure.json >/home/raza/goDash/DashApp/src/goDASH/_host_'+str(h)+'_band_'+str(b) +'_delay_'+str(d)+'_ploss_'+str(p)+ '/h_'+str(num)+'.txt && echo done_' + str(num) )
...
def  tc(s):
     os.system('sudo ./tc.sh')

if __name__ == '__main__':
    setLogLevel( 'info' )

    station, switch, ser , h, ba, de, pl =topology()
    u=0
    v=0
    x=1
    a=0
    ...
    if b==1:
       n = Process(target=cap2, args=(h,ba,de,pl,))   
       n.start()       
       d=1 
    

    if d==1:
       nnn = Process(target=tc, args=(ser,))   
       nnn.start()       
       ddd=1 
    ...
    if ddd==1:
       ...
       for k in range(h):
           print station[k]
           q = Process(target=test1, args=(station[k],h,ba,de,pl,))  
           q.start()
           q.join
           kk=1
